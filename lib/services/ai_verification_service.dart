...
class AIVerificationService {
  ...
  static const String _apiKey = 'YOUR_HUGGING_FACE_API_KEY'; // Get free API key from huggingface.co
  ...
  Future<bool> verifyTaskCompletion(String taskTitle, File imageFile) async {
    try {
      ...
      final isVerified = await _analyzeTaskCompletion(taskTitle, imageDescription);
  ...
  Future<String?> _getImageDescription(File imageFile) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Image description API Error: ${response.statusCode} - ${response.body}');
      }
  ...
  Future<bool> _analyzeTaskCompletion(String taskTitle, String imageDescription) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Text analysis API Error: ${response.statusCode} - ${response.body}');
      }
...
''';
  ...
  bool _fallbackVerification(String taskTitle, String imageDescription) {
    ...
    for (final taskWord in taskWords) {
      if (taskWord.length > 3) { // Only consider meaningful words
  ...
  String _getTaskContext(String taskTitle) {
    ...
    if (title.contains('exercise') || title.contains('workout') || title.contains('gym')) {
      ...
      return 'education';
    } else if (title.contains('clean') || title.contains('wash') || title.contains('organize')) {
      return 'cleaning';
    } else if (title.contains('cook') || title.contains('meal') || title.contains('food')) {
      return 'cooking';
    } else if (title.contains('work') || title.contains('project') || title.contains('meeting')) {
      return 'work';
    } else if (title.contains('walk') || title.contains('run') || title.contains('jog')) {
      return 'outdoor';
    }
  ...
  bool _checkContextMatch(String context, String description) {
    ...
    switch (context) {
      case 'fitness':
        return desc.contains('gym') || desc.contains('exercise') || 
               desc.contains('workout') || desc.contains('fitness') ||
               desc.contains('running') || desc.contains('weights');
      
      case 'education':
        return desc.contains('book') || desc.contains('study') || 
               desc.contains('reading') || desc.contains('desk') ||
               desc.contains('computer') || desc.contains('notes');
      
      case 'cleaning':
        return desc.contains('clean') || desc.contains('tidy') || 
               desc.contains('organized') || desc.contains('neat') ||
               desc.contains('vacuum') || desc.contains('wash');
      
      case 'cooking':
        return desc.contains('kitchen') || desc.contains('food') || 
               desc.contains('cooking') || desc.contains('meal') ||
               desc.contains('plate') || desc.contains('dish');
      
      case 'work':
        return desc.contains('computer') || desc.contains('desk') || 
               desc.contains('office') || desc.contains('laptop') ||
               desc.contains('meeting') || desc.contains('work');
      
      case 'outdoor':
        return desc.contains('outside') || desc.contains('park') || 
               desc.contains('street') || desc.contains('nature') ||
               desc.contains('walking') || desc.contains('running');
  ...
  Future<bool> verifyWithAlternativeModel(String taskTitle, File imageFile) async {
    try {
